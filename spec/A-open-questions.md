...
| # | Question | Status |
|---|---|---|
| A38 | There is **no L1 reference file** in `examples/`. L0 and L2 each have one, so the L1 column of Appendix C is the only one whose enforcement is measured against a fixture *derived* by `check_profiles.py` (the L0 file plus `fleet` and `safety`) rather than against a show anyone could fly. The derived fixture is validated before use, so the results are sound, but L1 is the profile most implementers will actually target and it deserves a real example: segment trajectories, a light program, a device profile and a takeoff grid. | open |
